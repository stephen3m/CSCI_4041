...
def merge_sort(A):
    sublistLength = 1 
    while (sublistLength < len(A)): 
        startL = 0
        while(startL < len(A)):  
            endL = min(startL + sublistLength - 1, len(A) - 1)
            endR = min(startL + (sublistLength*2) - 1, len(A) - 1)
            
            #Merge process
            numL = endL - startL + 1 
            numR = endR - endL
            leftList = [Email("", "")] * numL
            rightList = [Email("", "")] * numR
            for i in range(0, numL):
                leftList[i] = A[startL + i]
            for j in range(0, numR):
                rightList[j] = A[endL + j + 1]
            i = 0
            j = 0
            k = startL
